...
(async () => {
  ...
  const privateKey = new aptos.HexString(process.env.PRIVATEKEY);

  const account1 = new aptos.AptosAccount(privateKey.toUint8Array());
  console.log("Account Address fetched from pvt key =>",account1)
  ...
  const collectionName = "Aptos Dev NFT Collection";
  const tokenName = "Aptos Devnet NFT ";
  ...
  const txnHash1 = await tokenClient.createCollection(
    account1,
    collectionName,
    "Aptos City collection",
    "https://devnet.apscan.io/",
  ); // <:!:section_4
  ...
  const txnHash2 = await tokenClient.createToken(
    ...
    "Aptos City token",
    1,
    "https://www.todaynftnews.com/wp-content/uploads/2022/10/Be-on-the-lookout-for-Network-Scalability-as-Aptos-NFTs-arrive..jpg",
  ); // <:!:section_5
